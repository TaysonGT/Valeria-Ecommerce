...
const Cart:React.FC<Props> = ({show, setShow})=>{
    ...
    return (
        <div className={`fixed flex flex-col ${!show&&'pointer-events-none'} top-0 h-screen bg-white text-black left-full duration-300 z-[200] p-10 w-100 ${show&& '-translate-x-full'}`}>
            ...
            {cartItems.length>0?
                ...
                <div className='relative py-4 shrink-0 flex items-start shadow-soft text-sm select-none group border-b border-gray-200 overflow-hidden' key={item.product._id}>
                    ...
                    <div className='relative w-20 shrink-0 bg-[#F2F6F7]'>
                        <img className='w-full object-cover' src={item.product.imgs[0]?.url.replace('src/assets', '/test-images')||'/logo.png'} alt="" />
                    </div>
                    <div className='px-6 grow truncate min-w-0 text-base'>
                        ...
                        <div className='flex mt-2'>
                            ...
                            <input type="number" className='w-10 text-center bg-gray-100 outline-none' value={item.quantity} />
                        ...
                        <p className='mt-2'>${item.product.discountPrice||item.product.basePrice} x {item.quantity}</p>
                        <p className=''>Cost: ${(item.product.discountPrice||item.product.basePrice)*item.quantity}</p>
                    </div>
                </div>
                )}
                </>:
            ...
            <div className='flex text-nowrap mt-auto justify-between gap-4'>
                <button onClick={()=>{
                    nav('/cart')
                    setShow(false)
                ...
                <button onClick={()=>{
                    nav('/checkout')
                    setShow(false)
                    }} className='px-8 py-2 z-0 hover:bg-transparent bg-[#071c1f] border border-[#071c1f] duration-400 text-white hover:text-[#071c1f] text-lg rtl before:bg-[#071c1f] btn cursor-pointer'>Checkout</button>
            </div>
