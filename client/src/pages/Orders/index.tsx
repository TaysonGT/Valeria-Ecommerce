...
const OrdersPage = () => {
  ...
  return (
    <div className='min-h-screen bg-[#fcfcfc] p-2 w-full font-[Outfit]'>
      <div className='rounded-sm p-4 md:p-8 shadow-sm'>
        <div>
          <h1 className='text-3xl lg:text-4xl font-normal'>My Orders</h1>
          {error && <p className='mt-4 text-red-600'>{error}</p>}
        </div>
        
        <div className='flex w-full gap-2 md:gap-6 mt-4 md:mt-8 border-b border-gray-200 font-bold text-sm sm:text-base'>
          {tabs.map((tab) => (
            <button
              key={tab.value}
            ...
            >
              {tab.label} 
              <span className={`text-xs ${searchParams.get('status') === tab.value ? 'text-blue-600 bg-blue-50' : 'text-gray-600 bg-gray-100'} py-0 px-1 md:px-2 md:py-1 rounded-full text-xs font-light border border-[#d9d9d9]`}>{ordersTotals[tab.value as 'upcoming'|'delivered'|'cancelled']}</span>
            </button>
          ))}
        </div>
