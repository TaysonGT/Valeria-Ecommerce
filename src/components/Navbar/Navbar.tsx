...
const Navbar: React.FC = () => {
  ...
  return ( 
    <div className='bg-black relative text-white w-full'>
      <div className={'w-screen h-screen fixed top-full left-0 z-[99] bg-white opacity-0 duration-300 '+ (searchbar&& '-translate-y-full opacity-30')} 
      ...
      <div className={'bg-white text-black w-full top-0 absolute py-4 z-[100] -translate-y-full duration-300 '+ (searchbar&& 'translate-y-0')}>
        ...
        <form className='flex w-[90%] mx-auto gap-2'>
          <input className='border-b border-gray-700 focus:placeholder:opacity-0 duration-150 outline-none placeholder:text-gray-600 grow px-4 py-2' placeholder='Search Products...' type="search" name="search" id="" />
          <FaSearch className=''/>
        </form>
      </div>
      <div className='flex justify-between items-center w-[90%] m-auto py-4'>
            ...
            <div key={i} className='group-hover:text-[#418791] duration-150 relative group/primary select-none z-[9]  cursor-pointer'>
              ...
              <div className='absolute group-hover/primary:opacity-100 group-hover/primary:top-[100%] group-hover/primary:scale-y-100 origin-top scale-y-95 pointer-events-none group-hover/primary:pointer-events-auto opacity-0 top-[80%] bg-white text-black shadow-lg text-sm duration-400'>
                {link.children.map((child)=> 
                  <div className='relative border-t border-gray-400 text-nowrap group/secondary'>
                  ...
                  <>
                    <div className='absolute left-[90%] -top-[1px] shadow-lg opacity-0 group-hover/secondary:opacity-100 group-hover/secondary:left-[100%] bg-white text-black text-sm duration-400 '>
                      {child.children.map(grandchild=>
                        <Link to={grandchild.path} className='flex relative justify-between items-center p-4 border-t gap-3 border-gray-400 text-nowrap'>
                          {grandchild.text}
                        </Link>
                      )}
                    </div>
        ...
        <div className='flex items-center gap-6'>
          <MdOutlineSearch onClick={()=>setSearchbar(true)} className='cursor-pointer text-[25px] hover:scale-[1.15] duration-200'/>
          ...
          <Link className='group relative' to='/'>
            ...
            <span className='absolute -top-1 -left-2 text-[12px] bg-[#eab308] text-[#1e293b] font-bold rounded-full w-4.5 h-4.5 flex justify-center items-center'>0</span>
          </Link>
        </div>
      </div>
    </div>
